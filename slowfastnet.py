...
if __name__ == "__main__":
    ##Test the model
    input_tensor = torch.autograd.Variable(torch.rand(1, 3, 64, 224, 224)).cuda()
    model = SlowFast(Bottleneck, [3, 4, 6, 3],num_classes=200).cuda()
    before = model(input_tensor)
    
    state_dict = torch.load('slowfast50_best_fixed.pth') #request for pre-trained weights from lizfeyisayo@gmail.com
